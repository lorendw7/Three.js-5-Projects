...
//
// 【进阶·执行优先级】JS 把异步任务分两类，同步代码跑完后：
//   先清空所有【微任务 microtask】(Promise.then) → 再做一个【宏任务 macrotask】(setTimeout)。
//   所以即使 setTimeout 写 0 毫秒，也排在所有 Promise 回调之后。
//   记忆：同步 > 微任务(Promise) > 宏任务(setTimeout)。
//   （日常写 Three.js 加载用不到这么细，但面试常考、也能彻底解释"为什么 C 比 D 先"。）
